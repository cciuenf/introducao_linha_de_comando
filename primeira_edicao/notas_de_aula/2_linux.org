...
** root ou "/" (Raiz ou Barra)
   ...
   - Segue o padrão FHS (Filesystem Hierarchy Standard), mantido pela Linux Foundation
...
** /dev (Devices)
   ...
   - Possui arquivos que representam seu hardware, como:
     1. Webcam
     2. Microfone
     3. Fones de ouvido
     4. Teclado
...
** /lib, /lib32, /lib64 /libx32 (Library ou Biblioteca)
   ...
   - O nome das pastas representam a arquitetura a qual elas guardam os arquivos
...
** /opt (Optional ou Opcional)
   - Geralmente fabricantes que fazem computadores com Linux, guardam configurações aqui
...
** /sbin (System Binaries ou Binários de sistema)
   ...
   - Guarda os executáveis dos programas que precisam de permissões de admin para serem executados
...
** Caminhos (Paths)
Os caminhos (pastas) nos sistemas UNIX, são representamos da seguinte forma:
...
** Usuário
   ...
   - Usuários comuns possuem permissões distantes e restritas ao sistema
...
* Permissões
  ...
  coluna do comando ~ls~. Vamos destrinchar isso:
  ...
  Após esse primeiro caractere, os 9 seguintes são grupos de 3 em 3, sendo:
...
*** WMs (Window Managers ou Gerenciadores de Janelas)
    ...
    design inicial, você é quem vai montar ele pelas configurações.
...
* Distros
  ...
  Lembre-se que tudo (ou quase tudo) no mundo Linux é Open Source, ou seja: possui o
